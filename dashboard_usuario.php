...
if (!isset($_SESSION['role']) || $_SESSION['role'] != 'usuario' || !isset($_SESSION['org_id'])) {
...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-5 col-md-12">
            <h3>Enviar Relatório de Operação</h3>
            <?php echo $message; ?>
            <div class="card shadow-sm"><div class="card-body">
                <form action="process_transaction.php" method="POST">
                    ...
                    <button type="submit" class="btn btn-primary w-100">Enviar Relatório</button>
                </form>
            </div></div>
        </div>
        <div class="col-lg-7 col-md-12">
            <h3>Seus Relatórios</h3>
            <form action="dashboard_usuario.php" method="POST" class="mb-3 p-3 border rounded bg-light">
                <input type="hidden" name="action" value="filtrar">
                <div class="row g-2 align-items-end">
                    <div class="col-md-5"><label for="date_start" class="form-label small mb-1">De:</label><input type="date" class="form-control form-control-sm" name="date_start" value="<?php echo htmlspecialchars($date_start); ?>"></div>
                    <div class="col-md-5"><label for="date_end" class="form-label small mb-1">Até:</label><input type="date" class="form-control form-control-sm" name="date_end" value="<?php echo htmlspecialchars($date_end); ?>"></div>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th>Data</th><th>Depósito</th><th>Saque</th><th>Baú</th>
                            <th>Lucro</th><th>Sua Comissão</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                            // **** MODIFICADO: Adiciona AND org_id = ? ****
                            $stmt = $pdo->prepare("
                                SELECT * FROM relatorios 
                                WHERE id_usuario = ? AND org_id = ? AND (data BETWEEN ? AND ?)
                                ORDER BY data DESC
                            ");
                            $stmt->execute([$id_usuario_logado, $org_id, $date_start, $date_end_query]);
                            $relatorios = $stmt->fetchAll();
                            $total_lucro = 0; $total_comissao = 0;
                    ...
                    <tfoot class="table-group-divider">
                        <tr>
                            ...
                            <td class="text-success fw-bold">R$ <?php echo number_format($total_lucro, 2, ',', '.'); ?></td>
                            <td class="text-success fw-bold">R$ <?php echo number_format($total_comissao, 2, ',', '.'); ?></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
